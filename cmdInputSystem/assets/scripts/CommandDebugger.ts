...
import { CommandInput, InputKey, isInputKey, Command } from "./CommandInput";
...
export class CommandVisualizer extends Component {
  ...
  private lastTriggerCmd: Command = null;
  ...
  update() {
    ...
    if (this.lastTriggerCmd && this.lastTriggerCmd.sequence.length >= maxIdx) {
      ...
      text = this.cmdInput["formIdx"] === 0 ? "<b>雷法技能表</b>\n" : "<b>火法技能表</b>\n";
      for (const cmd of this.commands) {
        const state = this.states[cmd.name];
  ...
  private formatCommand(
    cmd: Command, stateIndex: number, isTriggered: boolean = false
  ): string {
  ...
  debugHandler (key: InputKey) {
    ...
    for (const cmd of this.commands) {
      ...
      const expected = this.cmdInput.resolveCommandKey(seq[state.index]);
